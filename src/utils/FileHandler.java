...
/**
 * Handles persistent data storage by reading and writing to a local text file.
 */
    ...
    public static void saveData(List<LibraryItem> items) {
        try (PrintWriter out = new PrintWriter(new FileWriter(DATA_FILE))) {
            for (LibraryItem item : items) {
                // Using | as a delimiter for safe parsing
                out.println(item.getType() + "|" + item.getId() + "|" + item.getTitle() + "|" +
                        item.getAuthor() + "|" + item.getYear() + "|" +
                        item.getCategory() + "|" + item.getStatus() + "|" + item.getBorrowedBy());
            }
        } catch (IOException e) {
    ...
    public static List<LibraryItem> loadData() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                LibraryItem item;
                if (p[0].equals("Book")) {
                    item = new Book(p[1], p[2], p[3], Integer.parseInt(p[4]), p[5]);
                } else if (p[0].equals("Magazine")) {
                    item = new Magazine(p[1], p[2], p[3], Integer.parseInt(p[4]), p[5]);
                } else {
                    item = new Journal(p[1], p[2], p[3], Integer.parseInt(p[4]), p[5]);
                }
