...
public class SearchSortPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        searchBtn.addActionListener(e -> {
            ...
            int index;
            if (isSortedByTitle) {
                // Use Binary Search if already sorted by title
                index = SearchEngine.binarySearchByTitle(manager.getInventory(), query);
            } else {
                // Use Recursive Search as a fallback
                index = manager.recursiveSearch(query, 0);
            }
